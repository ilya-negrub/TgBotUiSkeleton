...
{
    public class AddUserPermissionHandler(IHashRepository<UserHashEntity> _userRepository)
        : IYesNoQuestionHandler
    {
        public string GetQuestion(CallBackStrategyPath path)
        {
            ...
            return $"Добавить разрешение {permission.GetName()} пользователю {userName}?";
        }
        ...
        public async Task<BotRenderType> Processing(IBotContext context, CallBackStrategyPath path)
        {
            ...
            if (requestValue == "y")
            {
                ...
                _userRepository.Set(userId, x => x.Permission, permission.GetFieldId(), Permission.Allow);
